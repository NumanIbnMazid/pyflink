...
def conditional_demo():
    ...
    verified_ds = ds.filter(lambda x: x[0] % 2 == 0).name("filter_even").map(verify).name("add_verify")
    modified_ds = ds.filter(lambda x: x[0] % 2 != 0).name("filter_odd").map(modify).name("add_modify")
